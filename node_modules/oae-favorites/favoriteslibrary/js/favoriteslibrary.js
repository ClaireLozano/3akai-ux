    ...
    return function(uid, showSettings, widgetData) {
        ...
        var getFavoritesLibrary = function() {
            ...
            if ((widgetData.canAdd || widgetData.canManage) && !query) {
                initialContent = oae.api.util.template().render($('#favoriteslibrary-list-actions-template', $rootel));
            }

            var url = '/api/favorites/library/' + widgetData.context.id;
            if (query) {
                url = '/api/search/favorites-library/' + widgetData.context.id;
            }
            ...
            infinityScroll = $('.oae-list', $rootel).infiniteScroll(url, {
                ...
                'emptyListProcessor': function() {
                    oae.api.util.template().render($('#favoriteslibrary-noresults-template', $rootel), {
                        'query': query
                    }, $('.oae-list', $rootel));
                }
            });
        ...
        var setUpListHeader = function() {
            ...
            if (!oae.data.me.anon) {
                // If the user is logged in, they have the option to remove from favorite the items
                listHeaderActions.push({
                    ...
                    'icon': 'fa-star',
                    'label': oae.api.i18n.translate('__MSG__REMOVE_FROM_FAVORITES__', 'favoriteslibrary'),
                    'trigger': 'oae-trigger-removefavorite',
                    'data': {'resourceType': 'content'}
                });
                ...
                if (widgetData.canManage) {
                    ...
                    listHeaderActions.push({
                        'icon': 'fa-trash-o',
                        'label': oae.api.i18n.translate('__MSG__DELETE__', 'favoriteslibrary'),
            ...
            oae.api.util.template().render(
                $('#favoriteslibrary-list-header-template', $rootel), 
                {'actions': listHeaderActions}, 
                $('#favoriteslibrary-list-header', $rootel)
            );
        };
