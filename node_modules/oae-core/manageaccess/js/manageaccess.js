...
define(['jquery', 'underscore', 'oae.core', 'jquery.autosuggest'], function($, _, oae) {
    ...
    return function(uid) {
        ...
        var saveManageAccess = function() {
            ...
            if ($('#manageaccess-share-code-role').val() !== 'disable') {
                oae.api.group.createShareCode(widgetData.contextProfile.id, $('#manageaccess-share-code-role').val(), function(err, data) {
                    if (err) {
                        oae.api.util.notification(
                            oae.api.i18n.translate('__MSG__CREATE_SHARE_CODE_FAILED__'),
                            oae.api.i18n.translate(err.msg),
                            'error');
                        return;
                    } 
                    oae.api.util.notification(
                        oae.api.i18n.translate('__MSG__CREATE_SHARE_CODE_SUCCESS__', 'manageaccess'),
                        oae.api.i18n.translate('__MSG__SUCCESS_TO_CREATE_SHARE_CODE__', 'manageaccess'));
                });
            } else {
                oae.api.group.removeShareCode(widgetData.contextProfile.id, function(err, data) {
                    if (err) {
                        oae.api.util.notification(
                            oae.api.i18n.translate('__MSG__REMOVE_SHARE_CODE_FAILED__'),
                            oae.api.i18n.translate(err.msg),
                            'error');
                        return;
                    } 
                    oae.api.util.notification(
                        oae.api.i18n.translate('__MSG__REMOVE_SHARE_CODE_SUCCESS__', 'manageaccess'),
                        oae.api.i18n.translate('__MSG__SUCCESS_TO_REMOVE_SHARE_CODE__', 'manageaccess'));
                });
            }
        };
        ...
        /**
         * Renders the share code view.
         */
        var renderShareCode = function() {
            oae.api.util.template().render($('#manageaccess-share-code-template', $rootel), {
                'roles': widgetData.roles
            }, $('#manageaccess-share-code', $rootel));

            // Get value
            oae.api.group.getShareCode(widgetData.contextProfile.id, function(err, data) {
                if (!_.isEmpty(data)) {
                    $('#share-code').val(data.code);
                    $('#manageaccess-share-code-role').val(data.role);
                } else {
                    $('#manageaccess-share-code-role').val('disable');
                    $('#share-code').val('This function is disable ...');
                }
            });
        };
        ...
        var addBinding = function() {
            ...
            $rootel.on('change', '#manageaccess-share-code-role', function() {
                $('#manageaccess-overview-save', $rootel).prop('disabled', false);
            });
        };
